...
void    Mode::execute(User& client, std::vector<std::string> args)
{
    // hanling errors
    ...
    if (args.size() > 1)
    {
        ...
        else if (mode == "k" || mode == "+k" || mode == "-k")
        {
            ...
            if (mode != "-k")
            {
                if (channel->get_pass() != "")
                {
                    client.reply(ERR_KEYSET(client.getNickname(), mode));
                    return;
                }
            ...
            else
            {
                ...
                {
                    client.reply(ERR_BADCHANNELKEY(client.getNickname(), mode, ":invalid key"));
                    return ;
                }
            }
            client.reply(RPL_CHANNELMODEIS(channelName, channelName + static_cast<char>(1), mode));
        ...
        else if (mode == "o" || mode == "+o" || mode == "-o")
        {
            ...
            // nickname.erase(0, 1);
            User* ptr = channel->getClienByItstNick(nickname);
            if (!ptr)
            {
        ...
        else if (mode == "l" || mode == "+l" || mode == "-l")
        {
            if (mode != "-l")
            {
                ...
                if (new_limit < 1)
                {
                    client.reply(ERR_UNKNOWNMODE(client.getNickname(), mode, " :limit must be greater than 0"));
                    return;
                }
                if ((size_t)new_limit < channel->get_size())
                {
                    client.reply(ERR_UNKNOWNMODE(client.getNickname(), mode, " :number of clients is already greater than limit"));
                    return;
                }
        ...
        {
            client.reply(ERR_UNKNOWNMODE(client.getNickname(), mode, " :is unknown mode char to me"));
            return;
        }
    }
}